...
while(1){
    ...
    {
        mpu9250_register_read(reg_addr, tmp, 1);
        data[i] = tmp[0];
        reg_addr++;
    }
    ...
    for( int i = 0; i < 8; i++)
    {
        for( int j = 0; j < 8; j++)
        {
            ...
            //printf("%02d %X%X ", i * 8 + j, data[(i * 8 + j) * 2 + 1], data[(i * 8 + j) * 2]);
            //printf("%02d %.2f ", i * 8 + j, 0.25 * (data[(i * 8 + j) * 2 + 1] * 0x100 + data[(i * 8 + j) * 2]));
            printf("%.2f ",0.25 * (data[(i * 8 + j) * 2 + 1] * 0x100 + data[(i * 8 + j) * 2]));
        }
        printf("\n");
    }
